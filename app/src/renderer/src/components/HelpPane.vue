...
<ul><li><strong>黄色数字 = 停机期间消耗</strong>：软件没运行时那段余额下降，跨了多天无法按天归属 ⇒ 单独列「停机期间」，已计入「合计（含停机）」，但<strong>不计入</strong>日均、耗尽预估与预算判断。</li><li><strong>橙色「充值」= 当天余额反而增加了</strong>（疑似充值）：充值会掩盖消耗，所以那天算不出消耗，这一格不代表用量。</li><li><strong>预计耗尽</strong>：&lt; 3 天红、&lt; 14 天黄、&gt; 365 天绿（余量充足）。</li><li>页面上有「表格颜色说明」图例，单元格悬浮还会写明具体数值与原因。</li></ul>
<h3>6) 浅色主题</h3>
