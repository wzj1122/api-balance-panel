...
<h3>5) 平台用量统计</h3>
<ul><li>按平台类型 × 单位聚合（对应「每日使用状况」的账号级口径），支持近 7 / 30 天切换。</li><li><strong>积分和金额怎么比</strong>：两者数量级差很多（100 万积分 vs 几块钱），直接并排看数字会得出错误印象。所以本页会把各单位按「设置 → 成本折算」的折算率换成<strong>人民币估算值</strong>，对比条按折合人民币<strong>排序定宽</strong>，明细表也有「折合人民币」列（CSV 同步导出该列）。</li><li>默认折算率 <strong>111111</strong>（1 元 ≈ 11.11 万积分，即 <strong>100 万积分 ≈ 9 元</strong>），按 DeepSeek flash 高峰价反推（输入缓存命中 0.04 / 未命中 2 / 输出 8 元每百万 Token）；它只是比较用的估算基线，不是平台官方汇率，也不代表真实扣费。填 0 = 不折算（此时对比条退回按原值画，并提示"积分与金额不可比"）。</li><li>顶部信息卡：总消耗（分单位）、<strong>折合人民币（估算）</strong>、消耗最多平台（按折算值）、今日消耗。</li><li>平台消耗对比条形图 + 平台×日期明细表（含今日、7日均、合计、折合人民币），支持导出 CSV。</li></ul>
<h3>5.1) 表格里的颜色都是什么意思</h3>
<ul><li><strong>红色数字 = 停机期间消耗</strong>：软件没运行时那段余额下降，跨了多天无法按天归属 ⇒ 单独列「停机期间」，已计入「合计（含停机）」，但<strong>不计入</strong>日均、耗尽预估与预算判断。</li><li><strong>带虚线红字的「充值」= 当天余额反而增加了</strong>（疑似充值）：充值会掩盖消耗，所以那天算不出消耗，这一格不代表用量。</li><li><strong>预计耗尽</strong>：只有 &lt; 3 天会标红，其余保持普通字色。</li><li>鼠标停在单元格上会写明具体数值与原因（不再靠多种颜色区分，表格里只用红色一种强调色）。</li></ul>
<h3>6) 浅色主题</h3>
<ul><li>设置 → 显示 → 主题，可切换深色 / 浅色（点一下立即生效并保存）；这两项就是「主题外观」里的<strong>经典深色 / 经典浅色</strong>，两边是同一个设置，会实时同步。</li></ul>
<hr/>
<h2>七、监控与玩法（新功能）</h2>
...
<h3>7) 主题与背景</h3>
<ul><li>「设置 → 显示 → 主题」支持<strong>深色 / 浅色 / 跟随系统</strong>三种：选深色或浅色会直接切到对应的经典主题；选跟随系统则按 Windows 的深浅色，在经典深色与经典浅色之间自动切换（系统换色时界面实时跟着变）。</li><li>想让界面用设计师主题（如深空蓝 / 亚克力），到「主题外观」页点选即可，切换后「设置 → 显示 → 主题」会自动对应该主题。</li></ul>
<hr/>`
