...
<ul><li>在 <a href="https://platform.sensenova.cn/console" target="_blank" rel="noopener">SenseNova 控制台</a>登录一次即可；面板里选「商汤 日日新（SenseNova）」→ 点「登录并获取」，窗口会自动检测并关闭。</li><li>额度口径（与官方「账户总览」一致）：<strong>5 小时窗口</strong>（窗口到期自动重置，卡主数值/进度条取它）＋ <strong>周额度</strong>（7 天总量）＋ <strong>Flash-Lite 积分池</strong>（Flash-Lite 模型专属池，单独计量）。</li><li>注意：额度接口只认<strong>登录态</strong>（不认 API Key）。<strong>登录态真实寿命 ≈ 3 小时</strong>：服务端会话 Cookie 到期时间 = 登录时刻 + 3 小时，<strong>访问也不会顺延</strong>，平台也没有刷新会话的接口 —— 会话到期后只能重新登录（平台硬限制）。面板会在<strong>到期前 15 分钟弹通知</strong>提醒你，重登只需在账号编辑里点一下「只重新登录」。</li><li><strong>会话存活期内不用反复登录</strong>：面板后台每 5 分钟检查一次，会话还能用时自动换新 token；再次点「登录」时也会先复用它（平台认就不用输密码）。卡片上的「续期登录」按钮可手动续期，会话真失效时会自动弹出登录窗口。商汤 / 小米 MiMo 都适用，商汤受上面 3 小时硬上限约束。</li></ul>
<hr/>
