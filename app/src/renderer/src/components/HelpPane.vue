...
<h3>2) 每日使用状况统计</h3>
<ul><li>数据来自程序每次刷新自动记录的历史快照，无需额外开启；没数据时页面会提示。</li><li>口径：<strong>套餐型账号</strong>（如小米 MiMo 套餐）直接用「已用」增量；<strong>余额型账号</strong>按「剩余变化」估算（中途充值会掩盖消耗，仅供参考）。</li><li><strong>近 7 天</strong>：消耗信息卡 + 按单位柱状图 + 账号×日期明细表。</li><li><strong>近 30 天</strong>：日历形态——按自然月展示，格子颜色越深当天消耗越多（按主单位归一化）；每月顶部显示<strong>每月消耗合计、消耗最多的日期、消耗最多的账号</strong>；点击任意有数据的天，弹窗显示当天每个账号的用量明细和单位合计。</li></ul>
<h3>3) 余额耗尽预估</h3>
