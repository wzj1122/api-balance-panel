...
<h3>2) siliconflow</h3>
<ol><li>打开 <a href="https://cloud.siliconflow.cn/" target="_blank" rel="noopener">硅基流动控制台</a> 登录（没账号先注册）。</li><li>面板里选「siliconflow」，点「登录获取 Cookie」——会弹出登录窗口，登录完成后窗口会自动关闭，程序自动抓取 Cookie。</li><li>Cookie 失效后卡片会提示，点卡片上的「重新登录」（🔑）即可一步完成重新登录并自动刷新。</li><li>说明：官方已于 2026-08-14 下线 API Key 余额接口，所以这个平台必须用「登录」方式。</li></ol>
<h3>3) Xiaomi MIMO</h3>
...
<hr/>
<h2>三、日常使用</h2>
<ul><li><strong>自动刷新</strong>：默认每 5 分钟刷新一次（可在设置里调整 1 / 5 / 15 / 30 / 60 分钟）。程序最小化到托盘时是否继续刷新由「窗口隐藏时暂停」开关控制（默认暂停，回到前台立即补刷）。</li><li><strong>低余额提醒</strong>：每个账号可单独设置「剩余低于 X 时提醒」；触发后卡片余额变黄/显示「低余额」标记，并弹系统通知。</li><li><strong>单账号刷新</strong>：点卡片右上角箭头图标；失败卡片上有点击重试按钮。</li><li><strong>趋势图</strong>：点卡片右上角趋势按钮会弹出<strong>趋势悬浮窗</strong>（固定 420px 宽），显示模型、剩余余额与余额走势；横轴固定为 <strong>1 小时 / 6 小时 / 24 小时</strong> 三档可切换，打开时按最近使用自动选轴（近 3 小时有使用 → 1 小时；当日有使用 → 6 小时；当日没用 → 24 小时），点弹窗外部或 Esc 关闭。走势数据保存在本机。</li></ul>
<h2>四、常见问题</h2>
...
<h3>4) 新手引导与演示模式</h3>
<ul><li>首次启动（还没有账号时）会弹出新手引导，可以「添加第一个账号」或「先看演示模式」。</li><li><strong>演示模式</strong>内置 6 个示例账号与最近 30 天的示例数据，能完整体验卡片、日历、统计、报告、接口监控等全部功能；数据只存在内存里，<strong>不会写入你的配置、不发起任何请求</strong>，点「退出演示」即恢复。</li><li>各个新功能标题旁的「?」是即时说明，鼠标悬停即可看到这个功能是怎么算的。</li></ul>
<h3>5) 托盘与通知</h3>
