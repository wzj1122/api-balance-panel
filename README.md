...
一个常驻电脑上的小窗口：打开就能在一屏里看完所有 AI 平台账号的余额，余额低于阈值时自动变色提醒，不用再逐个登后台。
...
- **趋势悬浮窗**：点卡片趋势按钮弹出（固定 420px），显示模型 / 剩余余额 / 余额走势；横轴 **1 / 6 / 24 小时**可切换，按最近使用自动选轴，点外部关闭
- **每日使用状况**：按日消耗统计、日历热力图、导出 CSV（Excel 直接打开不乱码）；**停机期间消耗单独标记**（表格只用红色一种强调色，鼠标悬浮写明原因与数值），不污染日均与耗尽预估
- **平台用量**：按平台 × 单位聚合的消耗统计与对比图；**跨单位统一折算成金额比较**（积分型平台按可配置折算率折元——默认 100 万积分 ≈ 9 元，美元按实时汇率折元），展示仍用各自原单位原值；「今日消耗最多」同样按折算金额比较
...
1. 到 [Releases 页面](https://github.com/wzj1122/api-balance-panel/releases) 下载最新安装包（约 130 MB）。GitHub 附件名不支持中文，实际下载文件名为 **`API.Setup.<版本号>.exe`**（Releases 页最新发布版为 `API.Setup.1.3.0.exe`，源码最新版 v1.5.0 见 `main` 分支 / 本地打包产物 `API 余额面板 Setup 1.5.0.exe`；安装后的程序/快捷方式名为「API 余额面板」）。
...
1. 打开程序 → 右上角「添加账号」，选择平台类型：
   - **API Key 型**（DeepSeek）：粘贴 `sk-` 开头的 Key（可加 `base_url`）；
   - **登录型**（Xiaomi MIMO / MiniMax / bigmodel / siliconflow / SenseNova）：点「登录」按钮，在弹出的窗口里登录，完成后自动抓取凭据并关窗；
   - **AccessKey 型**（阿里云百炼）：RAM 控制台创建只读 AccessKey，格式 `AccessKey ID|AccessKey Secret`。
2. 保存后程序立即刷新一次，卡片显示余额；点卡片右上角图标可弹出趋势 / 刷新 / 编辑。
...
| 提示「密钥错了或过期了」 | DeepSeek 检查 Key；Xiaomi MIMO / siliconflow / bigmodel / MiniMax / SenseNova 点「重新登录」 |
...
| SenseNova 卡片提示「登录态已过期」 | 额度接口只认登录态（约 3 小时绝对到期、无法续期），到期前 15 分钟会弹提醒；点卡片的「重新登录」再登一次即可 |
