...
- **平台用量**：按平台 × 单位聚合的消耗统计与对比图；**积分与金额按可配置折算率放到同一把尺子上比较**（默认按 DeepSeek 高峰价反推：100 万积分 ≈ 9 元），新增「折合人民币」列与统一口径的总额
...
1. 到 [Releases 页面](https://github.com/wzj1122/api-balance-panel/releases) 下载最新安装包（约 130 MB）。GitHub 附件名不支持中文，实际下载文件名为 **`API.Setup.1.3.0.exe`**（当前最新发布版；安装后的程序/快捷方式名为「API 余额面板」，本地绿色版文件名为 `API 余额面板.exe`）。未发布安装包的中间版本（v1.1.1 / v1.1.2 等授权与文档更新）源码以仓库 `main` 分支为准。
